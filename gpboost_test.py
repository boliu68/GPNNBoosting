...
if __name__ == "__main__":
    ...
    depth = [1,2,3,4]
    reg = [[1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 0.1][cpu_id]]
    ...
    for tr, tst in kf:
        ...
        print "CPU ID:%d, CV:%d, Lasso, NNZ:%d, Tr Log Loss:%f, Tst Log Loss:%f, Lasso TR %s:%f, TST %s:%f" % (
            cpu_id, cv_id
            np.sum(ls.coef_!=0),
    ...
    print res
    pickle.dump([lasso_result_list, res], open("cpu%d.pkl" % cpu_id, "w"))
